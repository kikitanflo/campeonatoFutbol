...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Primero debemos eliminar los goles y eventos asociados al jugador para mantener la integridad (o usar cascada)
    // Como los goles están en la tabla jugadores no hay problema, pero eventos_partido sí depende de él
    await db.query('DELETE FROM eventos_partido WHERE jugador_id = ?', [jugadorId]);
    
    // Eliminar jugador
    await db.query('DELETE FROM jugadores WHERE id = ?', [jugadorId]);
    ...
    throw createError({ statusCode: 500, statusMessage: 'Error interno del servidor al eliminar jugador' });
  }
});
